...
export class FileProcessor {
  ...
  public convertFile = async (): Promise<void> => {
    if (this.source.targetFile && fs.existsSync(this.source.targetFile)) {
      ...
      const stream = fs.createReadStream(this.source.targetFile, {
        encoding: "utf-8",
        start: 0,
        end: 20,
      });
      ...
      symbols.forEach((symbol) => {
        if (
          ...
          const path = symbol.symbolPath.filter((entry) => {
            return (
              entry instanceof ClassSymbol ||
              entry instanceof InterfaceSymbol ||
  ...
  private processCompilationUnit = (
    ...
    if (typeof this.configuration.options?.prefix === "string") {
      prefix = () => {
        return this.configuration.options?.prefix as string;
      };
  ...
  private processTypeDeclarations = (
    ...
    list.forEach((context) => {
      ...
      if (context.classDeclaration()) {
        const details = this.processClassDeclaration(
        ...
        const details = this.processInterfaceDeclaration(
  ...
  private processClassOrInterfaceModifier = (
    builder: java.lang.StringBuilder,
    context: ClassOrInterfaceModifierContext | null,
    parentType: RelatedElement
  ): ModifierType => {
  ...
  private processElementValue = (
    builder: java.lang.StringBuilder,
    context: ElementValueContext | null
  ): void => {
    if (context) {
  ...
  private processElementValueArrayInitializer = (
    builder: java.lang.StringBuilder,
    context: ElementValueArrayInitializerContext | null
  ): void => {
    if (context) {
  ...
  private processClassDeclaration = (
    context: ClassDeclarationContext | null,
    prefix: string,
    modifiers: Set<string>,
    extraCtorParams: ExtraParameters | null
  ...
  private processClassBody = (
    ...
    const containsAbstract = this.processClassBodyDeclarations(
      builder,
  ...
  private processClassBodyDeclarations = (
    builder: java.lang.StringBuilder,
    list: ClassBodyDeclarationContext[],
    extraCtorParams: ExtraParameters | null
  ): boolean => {
    ...
    list.forEach((context) => {
      if (context.SEMI()) {
        ...
        if (context.memberDeclaration()) {
          const detailList = this.processMemberDeclaration(
  ...
  private processModifier = (
    ...
    if (context.classOrInterfaceModifier()) {
      result = this.processClassOrInterfaceModifier(
        builder,
  ...
  private processMemberDeclaration = (
    context: MemberDeclarationContext | null,
    prefix: string,
    modifiers: Set<string>,
    extraCtorParams: ExtraParameters | null
  ): ITypeMemberDetails[] => {
    ...
    switch (firstChild.ruleIndex) {
      case JavaParser.RULE_jsniMethodDeclaration: {
        const details = this.processJsniMethodDeclaration(
          firstChild as JsniMethodDeclarationContext
        );
        if (details) {
      ...
      case JavaParser.RULE_constructorDeclaration: {
        const details = this.processConstructorDeclaration(
          extraCtorParams,
      ...
      case JavaParser.RULE_genericConstructorDeclaration: {
        const details = this.processGenericConstructorDeclaration(
          extraCtorParams,
  ...
  private processMethodDeclaration = (
    context: MethodDeclarationContext | null,
    genericParams?: java.lang.StringBuilder
  ): ITypeMemberDetails | undefined => {
    if (!context) {
      return undefined;
    }
  ...
  private processFormalParameterList = (
    details: ITypeMemberDetails,
    context: FormalParameterListContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processGenericMethodDeclaration = (
    context: GenericMethodDeclarationContext | null
  ): ITypeMemberDetails | undefined => {
    if (!context) {
      return undefined;
    }
  ...
  private processFieldDeclaration = (
    context: FieldDeclarationContext | null,
    modifiers: Set<string>
  ): ITypeMemberDetails[] => {
    if (!context) {
      return [];
  ...
  private processConstructorDeclaration = (
    extraCtorParams: ExtraParameters | null,
    context: ConstructorDeclarationContext | null
  ): ITypeMemberDetails | undefined => {
    if (!context) {
      return undefined;
    }
  ...
  private processGenericConstructorDeclaration = (
    extraCtorParams: ExtraParameters | null,
    context: GenericConstructorDeclarationContext | null
  ): ITypeMemberDetails | undefined => {
    if (!context) {
      return undefined;
    }
  ...
  private processTypeParameters = (
    builder: java.lang.StringBuilder,
    context: TypeParametersContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processInterfaceDeclaration = (
    context: InterfaceDeclarationContext | null,
    prefix: string,
    doExport: boolean
  ): ITypeMemberDetails | undefined => {
    if (!context) {
  ...
  private processInterfaceBody = (
    ...
    context: InterfaceBodyContext
  ): void => {
    this.getContent(builder, context.LBRACE());
    this.processInterfaceBodyDeclarations(
      builder,
  ...
  private processInterfaceBodyDeclarations = (
    builder: java.lang.StringBuilder,
    isTypescriptCompatible: boolean,
    list: InterfaceBodyDeclarationContext[]
  ): boolean => {
    const members: ITypeMemberDetails[] = [];
    ...
    list.forEach((context) => {
      if (!context.SEMI()) {
        ...
        const details = this.processInterfaceMemberDeclaration(
          context.interfaceMemberDeclaration(),
          `${prefix.toString()}`,
          modifiers,
          isTypescriptCompatible
        );
  ...
  private processInterfaceMemberDeclaration = (
    context: InterfaceMemberDeclarationContext | null,
    prefix: string,
    modifiers: Set<string>,
    isTypescriptCompatible: boolean
  ): ITypeMemberDetails | undefined => {
    ...
    switch (firstChild.ruleIndex) {
      ...
      case JavaParser.RULE_interfaceMethodDeclaration: {
        this.processInterfaceMethodDeclaration(
          result,
          firstChild as InterfaceMethodDeclarationContext,
          isTypescriptCompatible
        );
      ...
      case JavaParser.RULE_genericInterfaceMethodDeclaration: {
        this.processGenericInterfaceMethodDeclaration(
          result,
      ...
      case JavaParser.RULE_interfaceDeclaration: {
        return this.processInterfaceDeclaration(
          firstChild as InterfaceDeclarationContext,
          prefix,
          modifiers.has("public")
        );
      }
      ...
      case JavaParser.RULE_annotationTypeDeclaration: {
        return this.processAnnotationTypeDeclaration(
      ...
      case JavaParser.RULE_classDeclaration: {
        return this.processClassDeclaration(
          firstChild as ClassDeclarationContext,
          prefix,
          modifiers,
          null
        );
  ...
  private processConstDeclaration = (
    details: ITypeMemberDetails,
    context: ConstDeclarationContext
  ): void => {
    ...
    while (true) {
      let child = context.getChild(index++);
      this.processConstantDeclarator(
        details.bodyContent,
        child as ConstantDeclaratorContext,
        `${type.toString()}`
      );
  ...
  private processConstantDeclarator = (
    builder: java.lang.StringBuilder,
    context: ConstantDeclaratorContext,
    type: string
  ): void => {
  ...
  private processInterfaceMethodDeclaration = (
    details: ITypeMemberDetails,
    context: InterfaceMethodDeclarationContext,
    isTypescriptCompatible: boolean
  ): void => {
    ...
    this.processInterfaceCommonBodyDeclaration(
      details,
  ...
  private processGenericInterfaceMethodDeclaration = (
    details: ITypeMemberDetails,
    context: GenericInterfaceMethodDeclarationContext,
    isTypescriptCompatible: boolean
  ): void => {
    // Ignore all method modifiers. Interface members in TS have no modifiers.
    context.interfaceMethodModifier().forEach((modifier) => {
  ...
  private processInterfaceCommonBodyDeclaration = (
    details: ITypeMemberDetails,
    context: InterfaceCommonBodyDeclarationContext,
    isTypescriptCompatible: boolean
  ): void => {
  ...
  private processAnnotationTypeDeclaration = (
    context: AnnotationTypeDeclarationContext | null
  ): ITypeMemberDetails | undefined => {
    if (!context) {
      return undefined;
    }
  ...
  private processEnumDeclaration = (
    context: EnumDeclarationContext | null,
    modifiers: Set<string>
  ): ITypeMemberDetails | undefined => {
    if (!context) {
      return undefined;
    ...
    if (declarations) {
      ...
      this.processClassBodyDeclarations(
        localBuilder,
  ...
  private processEnumConstants = (
    builder: java.lang.StringBuilder,
    context: EnumConstantsContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processLocalVariableDeclaration = (
    builder: java.lang.StringBuilder,
    context: LocalVariableDeclarationContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processVariableModifier = (
    builder: java.lang.StringBuilder,
    context: VariableModifierContext
  ): void => {
    if (context.FINAL()) {
  ...
  private processVariableDeclarators = (
    context: VariableDeclaratorsContext,
    ...
    while (true) {
      ...
      this.processVariableDeclarator(
        details.bodyContent,
        child as VariableDeclaratorContext,
        type,
        makeOptional
      );
      result.push(details);
  ...
  private processVariableInitializer = (
    builder: java.lang.StringBuilder,
    context: VariableInitializerContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processArrayInitializer = (
    builder: java.lang.StringBuilder,
    context: ArrayInitializerContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processParExpression = (
    builder: java.lang.StringBuilder,
    context: ParExpressionContext | null
  ): ISymbolInfo | null => {
    if (!context) {
      return null;
    }
              ...
              case JavaLexer.DOT: {
                if (context.identifier()) {
                  ...
                  if (call) {
                    ...
                    builder.append(firstExpression);
                    this.getContent(builder, context.DOT());
                    this.processExplicitGenericInvocation(
                      builder,
  ...
  private processExplicitGenericInvocation = (
    builder: java.lang.StringBuilder,
    context: ExplicitGenericInvocationContext | null
  ): void => {
    this.getContent(builder, context);
  };
  ...
  private processSuperSuffix = (
    builder: java.lang.StringBuilder,
    context: SuperSuffixContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processNonWildcardTypeArguments = (
    builder: java.lang.StringBuilder,
    context: NonWildcardTypeArgumentsContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processInnerCreator = (
    builder: java.lang.StringBuilder,
    context: InnerCreatorContext | null
  ): void => {
    if (!context) {
      return;
    }

    this.processNonWildcardTypeArgumentsOrDiamond(
      builder,
      context.nonWildcardTypeArgumentsOrDiamond()
    );
  ...
  private processClassCreatorRest = (
    builder: java.lang.StringBuilder,
    context: ClassCreatorRestContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processNonWildcardTypeArgumentsOrDiamond = (
    builder: java.lang.StringBuilder,
    context: NonWildcardTypeArgumentsOrDiamondContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processLambdaExpression = (
    builder: java.lang.StringBuilder,
    context: LambdaExpressionContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processLambdaParameters = (
    builder: java.lang.StringBuilder,
    context: LambdaParametersContext
  ): void => {
  ...
  private processMethodCall = (
    builder: java.lang.StringBuilder,
    context: MethodCallContext | null,
    instance: ISymbolInfo | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processExpressionList = (
    builder: java.lang.StringBuilder,
    context: ExpressionListContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processCreatedName = (
    ...
    while (index < context.getChildCount()) {
      ...
      if (child instanceof TypeArgumentsOrDiamondContext) {
        ...
        if (child.getText() !== "<>") {
          // Using .text here, as that leaves out all white spaces.
      ...
      switch ((firstChild as ParserRuleContext).ruleIndex) {
        ...
        default: {
          ...
          if (context.THIS()) {
            ...
            this.processExplicitGenericInvocationSuffix(
              builder,
              context.explicitGenericInvocationSuffix()
            );
          }
          break;
        }
  ...
  private processExplicitGenericInvocationSuffix = (
    builder: java.lang.StringBuilder,
    context: ExplicitGenericInvocationSuffixContext | null
  ): void => {
    if (!context) {
      return;
    }
      ...
      switch (id) {
        ...
        case JavaLexer.TRY: {
          ...
          if (context.resourceSpecification()) {
            ...
            const names = this.processResourceSpecification(
              builder,
              context.resourceSpecification()
            );
            builder.append("\ntry {\n\ttry ");
            this.processBlock({ builder, context: context.block() });
            builder.append(
  ...
  private processResourceSpecification = (
    builder: java.lang.StringBuilder,
    context: ResourceSpecificationContext | null
  ): string[] => {
    if (!context) {
      return [];
    }
  ...
  private processSwitchBlockStatementGroup = (
    ...
    const needBraces =
      context.blockStatement().length > 1 ||
      !context.blockStatement(0)?.statement() ||
  ...
  private processFinallyBlock = (
    builder: java.lang.StringBuilder,
    context: FinallyBlockContext | null,
    extra?: string
  ): void => {
    if (!context) {
      return;
  ...
  private processForControl = (
    builder: java.lang.StringBuilder,
    context: ForControlContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processEnhancedForControl = (
    builder: java.lang.StringBuilder,
    context: EnhancedForControlContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processVariableDeclaratorId = (
    builder: java.lang.StringBuilder,
    context: VariableDeclaratorIdContext
  ): void => {
    this.getContent(builder, context);
  };
  ...
  private processLocalTypeDeclaration = (
    builder: java.lang.StringBuilder,
    context: LocalTypeDeclarationContext | null
  ): void => {
    this.getContent(builder, context, true);
  };
  ...
  private processTypeTypeOrVoid = (
    builder: java.lang.StringBuilder,
    context: TypeTypeOrVoidContext | null
  ): boolean => {
    if (!context) {
      return false;
    }
  ...
  private processClassOrInterfaceType = (
    builder: java.lang.StringBuilder,
    context: ClassOrInterfaceTypeContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processTypeArguments = (
    builder: java.lang.StringBuilder,
    context: TypeArgumentsContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processPrimitiveType = (
    builder: java.lang.StringBuilder,
    context: PrimitiveTypeContext | null
  ): void => {
    if (!context) {
      return;
    }
  ...
  private processBodyMembers = (
    ...
    if (this.memberOrdering) {
      members = this.memberOrdering.apply(
        members,
        this.typeStack.peek().type
      ) as ITypeMemberDetails[];
    }
    ...
    while (true) {
      ...
      switch (member.type) {
        ...
        case MemberType.Method: {
          members = this.processConstructorAndMethodMembers(
            builder,
            member,
            members,
            generatedMembers,
  ...
  private processConstructorAndMethodMembers(
    ...
    if (needOverloadHandling) {
      ...
      overloads = members.filter((candidate) => {
        return (
          candidate.name === name &&
          !candidate.modifiers?.has("abstract") &&
    ...
    if (overloads.length > 0) {
      ...
      if (staticCount === 0 || staticCount === overloads.length) {
        ...
        if (member.type === MemberType.Constructor) {
          ...
          builder.append(
            `\n${modifier}` +
              `${member.nameWhitespace ?? ""}${member.name ?? "unknown"}${
  ...
  private getRangeCommented = (
    builder: java.lang.StringBuilder,
    start?: ParserRuleContext | TerminalNode,
    stop?: ParserRuleContext | TerminalNode
  ): void => {
    if (!start || !stop) {
      return;
    }
  ...
  private registerDefaultImports(): void {
    ...
    java.children.forEach((child) => {
      if (child.name === "lang") {
        // Looking for all symbols in java.lang.
  ...
  private overridesMethod = (
    context: MemberDeclarationContext,
    details: ITypeMemberDetails
  ): boolean => {
    if (!details.name) {
      return false;
